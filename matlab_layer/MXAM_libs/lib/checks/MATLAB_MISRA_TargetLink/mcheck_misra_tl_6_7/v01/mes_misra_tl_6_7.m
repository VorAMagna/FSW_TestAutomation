% MES check: misra_tl_6_7 - Avoiding rescaling of input signals to
% LookupTables
%
% function [result,cr_items,r_stats] = mes_misra_tl_6_7(system, cmd_s)
%
% RETURN PARAMETERS:
% ==================
...
% MES CHECK:
% ==========
% misra_tl_6_7 - Avoiding rescaling of input signals to TargetLink LookupTables
%
% Advice: Care must be taken to ensure that the block output preceding the
% input to a TargetLink Lookup table has a value that can be represented by the
% scaling of the input axis without causing an overflow or underflow.
%
% PASS/FAIL CRITERIA:
% ===================
% The check failes if the data range of input signal can not be covered by
% the data range of input axis of the look up table, or the input signal
% comes from a simulink block, whose scaling info is not available.
%
% CHECK PARAMETERS:
% =================
% MISRA.check67.trustUserSpecifiedRange:
%   DATATYPE: 
%       boolean
%   DEFAULT VALUE:
%       false (0)
...
function [result,cr_items,r_stats]=mes_misra_tl_6_7(system,cmd_s) %#ok<INUSD>
