% MES check: misra_tl_7_6 - Correct declaration of external variables
%
% function [result,cr_items,r_stats] = mes_misra_tl_7_6(system, cmd_s)
%
% RETURN PARAMETERS:
% ==================
...
% External variables should be declared in one and only one
% file.
% If a variable is declared as extern and TargetLink is unaware of its
% origin then a declaration for this variable might be generated in multiple
% locations. To avoid this issue and ensure that the generated code adheres
% to MISRA C Rule 8.8, the external variable should be specified in the
% Data Dictionary and have its origin speicified via the module entry.
%
% PASS/FAIL CRITERIA:
% ===================
% A fail is given to each TargetLink block, whose property *.class(gain.class, out.class...)
% is not empty and the entry storage of the class specified by this property 
% is extern and the property *.Variable of the block is empty or the module/moduleRef 
% entry of the variable specified by property *.variable is empty, or all 
% the entry module/moduleRef of all variables used by all the blocks in this model is
% not identical (one and only one file).
%
% Note: this check only checks all the TL blocks, which has at least the
% property 'output.class'.
%
% ***********************************************************
% copyright:    Model Engineering Solutions
% Date:         $Date: 2019/03/29 12:54:19CET $
% Revision:     $Rev: 6772 $
% ***********************************************************
