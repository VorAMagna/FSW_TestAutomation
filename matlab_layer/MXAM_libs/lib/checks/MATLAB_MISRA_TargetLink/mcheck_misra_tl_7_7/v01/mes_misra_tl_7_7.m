% MES check: misra_tl_7_7 - Use of Include Statements in Custom Code Block
%
% function [result,cr_items,r_stats] = mes_misra_tl_7_7(system, cmd_s)
%
% RETURN PARAMETERS:
% ==================
...
%                .linkaction: hyperref string linking to the faulty/fixed
%                items 
...
% Include statement should not be used in custom code block.
% An addfile block should be used instead.
% Autofix will delete the #include statement and add a addfile block in the
% same level as the custom code block (at the bottom of the subsystem).
%
% PASS/FAIL CRITERIA:
% ===================
% A fail is given to each Custom Code block, if the custom code file
% has #include statements in all the sections except the fxp_header and
% flp_header sections (in this case the include file is inserted in the
% head file of the generated code).
%
% Note: the file specified in the custom code block must be in the current
% path or in the search path of matlab.
%
% ************************************************************************
