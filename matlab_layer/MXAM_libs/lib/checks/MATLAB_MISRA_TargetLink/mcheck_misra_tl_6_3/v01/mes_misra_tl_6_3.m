% MES check: misra_tl_6_3 - Use of the Math block sqrt function for
% fixed-point code generation
%
% function [result,cr_items,r_stats] = mes_misra_tl_6_3(system, cmd_s)
%
% RETURN PARAMETERS:
% ==================
...
% MES CHECK:
% ==========
% misra_tl_6_3 - Use of the Math block sqrt function for
% fixed-point code generation
%
% The sqrt function of the Math block should not be used for fixed-point
% code generation if compliance with a maximum number of MISRA C rules is required.
%
% The sqrt function of this block violates a number of MISRA rules when
% used for fixed-point code generation so it should be avoided.
%
% Note:This rule does not imply that use of this block (resulting in
% noncompliance with the above rules) will result in unsafe code.
%
% PASS/FAIL CRITERIA:
% ==================
% A Math block which function is set to sqrt and which input and output
% datatype are fixed-point (=int) get a fail, otherwise it will pass.
% (if both of the input and output are fixed point, then TL will use
% intsqrt, which violate the Misra rule)
% ************************************************************************
