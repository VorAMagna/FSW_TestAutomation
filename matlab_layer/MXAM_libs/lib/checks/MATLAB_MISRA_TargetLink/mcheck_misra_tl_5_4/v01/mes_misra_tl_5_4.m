% MES check: misra_tl_5_4 - Use of From and Goto across TargetLink subsystems
%
% function [result,cr_items,r_stats] = mes_misra_tl_5_4(system, cmd_s)
%
% RETURN PARAMETERS:
% ==================
...
% MES CHECK: 
% ==========
% misra_tl_5_4 - Use of From and Goto across TargetLink subsystems
%
% From and Goto blocks must not be used to exchange data across TargetLink 
% subsystems boundaries. In addition, they should only be used in those
% cases where their relationship is transparent, for example within the 
% same subsystem.
%
% Data exchange via the above blocks across TragetLink subsystems lead to
% ill-defined interfaces. In addition, code generation with TargetLink is
% performed for all TargetLink subsystems individually, requiring that
% interface variables at the boundary of a TargetLink subsystem are
% properly defined.
%
% PASS/FAIL CRITERIA
% ==================
% A fail is given to From and Goto blocks which are used across the 
% tl-toplevel subsystems boundaries (tag visibility=scoped and global)
% and have the same tags. Otherwise a pass is given.
%
%
% ************************************************************************
...
function [result,cr_items,r_stats]=mes_misra_tl_5_4(system,cmd_s) %#ok<INUSD>
