% MES check: ds_0051 - Restrictions with regard to the use of blocks from
% the sources library.
%
% function [result,cr_items,r_stats] = mes_ds_0051(system, cmd_s)
%
% RETURN PARAMETERS:
% ==================
...
% MES CHECK: 
% ==========
% ds_0051 - Restrictions with regard to the use of blocks from the
% sources library.
%
% This check tests if only allowed block of sources library is used in
% model.
%
% Rationale:
% The unsupported blocks in this library are time-dependent sources which
% usually serve as input signals for a controller and are not part of the
% application algorithm itself.
%
% PASS/FALL CRITERIA:
% ================
% A pass is given if no block of sources library, except the blocks
% given in the parameter dSPACE.checkds0051.AllowedBlocks, is used,
% otherwise fail is given.
%
% Note: The blocks listet in dSPACE.checkds0051.AllowedBlocks must exactly
% be the same as the corresponding block in the library (i.e. including
% linebreaks, spaces)
%
% CHECK PARAMETERS:
% =================
% dSPACE.checkds0051.BeingCheckedSLLib:
%   DATATYPE: 
%       string
%   DEFAULT VALUE:
%       'simulink/Sources'
...
% 
% dSPACE.checkds0051.AllowedBlocks:
%   DATATYPE: 
%       cellstring
%   DEFAULT VALUE:
...
function [result,cr_items,r_stats] = mes_ds_0051(varargin)
