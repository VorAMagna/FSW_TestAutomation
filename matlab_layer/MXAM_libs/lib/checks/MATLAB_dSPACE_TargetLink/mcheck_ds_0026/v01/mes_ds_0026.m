% MES check: ds_0026 - Restrictions with regard to the use of blocks from
% the lookup tables library.
%
% function [result,cr_items,r_stats] = mes_ds_0026(system, cmd_s)
%
% RETURN PARAMETERS:
% ==================
...
% MES CHECK: 
% ==========
% ds_0026 - Restrictions with regard to the use of blocks from the
% lookup tables library.
%
% This check tests if only allowed block of lookup tables library is used in
% model.
%
% Rationale:
% TargetLink supports only a restricted subset of blocks to ensure the
% generation of efficient code. Please note that the Prelookup and
% Interpolation using Prelookup blocks introduced newly with R2006b are
% currently not supported by TargetLink. They are supposed to replace the
% Interpolation (n-D) using PreLookup and PreLookup Index Search block in
% future releases.
%
% PASS/FALL CRITERIA:
% ================
% A pass is given if no block of lookup tables library, except the blocks
% given in the parameter dSPACE.checkds0026.AllowedBlocks, is used,
% otherwise fail is given.
%
% Note: The blocks listet in dSPACE.checkds0026.AllowedBlocks must exactly
% be the same as the corresponding block in the library (i.e. including
% linebreaks, spaces)
% 
% CHECK PARAMETERS:
% =================
% dSPACE.checkds0026.BeingCheckedSLLib:
%   DATATYPE: 
%       string
%   DEFAULT VALUE:
...
% 
% dSPACE.checkds0026.AllowedBlocks:
%   DATATYPE: 
%       cellstring
%   DEFAULT VALUE:
%       {['simulink/Lookup' char(10) 'Tables/Direct Lookup' char(10) 'Table (n-D)'];...
...
%        ['simulink/Lookup' char(10) 'Tables/Prelookup']}
%   DESCRIPTION:
...
function [result,cr_items,r_stats] = mes_ds_0026(varargin)
