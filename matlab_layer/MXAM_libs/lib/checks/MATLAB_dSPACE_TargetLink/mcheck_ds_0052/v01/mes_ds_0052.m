% MES check: ds_0052 - Restrictions with regard to the use of blocks from
% the user-defined functions library.
%
% function [result,cr_items,r_stats] = mes_ds_0052(system, cmd_s)
%
% RETURN PARAMETERS:
% ==================
...
% MES CHECK: 
% ==========
% ds_0052 - Restrictions with regard to the use of blocks from the
% user-defined functions library.
%
% This check tests if only allowed block of user-defined functions library is used in
% model.
%
% Rationale:
% TargetLink supports only a restricted subset of blocks to ensure the
% generation of efficient code. If legacy code has to be integrated, the
% TargetLink Custom Code block should be used (instead of an S-Function
% block), as this also supports the generation of efficient production
% code.
%
% PASS/FALL CRITERIA:
% ================
% A pass is given if no block of user-defined functions library, except the blocks
% given in the parameter dSPACE.checkds0052.AllowedBlocks, is used,
% otherwise fail is given.
%
% Note: The blocks listet in dSPACE.checkds0052.AllowedBlocks must exactly
% be the same as the corresponding block in the library (i.e. including
% linebreaks, spaces)
% 
% CHECK PARAMETERS:
% =================
% dSPACE.checkds0052.BeingCheckedSLLib:
%   DATATYPE: 
%       string
%   DEFAULT VALUE:
...
% 
% dSPACE.checkds0052.AllowedBlocks:
%   DATATYPE: 
%       cellstring
%   DEFAULT VALUE:
%       {['simulink/User-Defined' char(10) 'Functions/Fcn']}
%   DESCRIPTION:
%       Allowed blocks within this library.
%
% ***********************************************************
% Copyright:    Model Engineering Solutions
% Date:         $Date: 2020/07/31 16:05:17CEST $
% Revision:     $Rev: 6835 $
% ***********************************************************

function [result,cr_items,r_stats] = mes_ds_0052(varargin)
