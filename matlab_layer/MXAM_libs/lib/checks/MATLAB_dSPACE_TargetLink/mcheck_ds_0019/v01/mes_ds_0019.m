% MES check: ds_0019 - Restrictions with regard to the use of blocks from
% the continuous library.
%
% function [result,cr_items,r_stats] = mes_ds_0019(system, cmd_s)
%
% RETURN PARAMETERS:
% ==================
...
% MES CHECK: 
% ==========
% ds_0019 - Restrictions with regard to the use of blocks from the
% continuous library.
%
% This check tests if only allowed block of continuous library is used in
% model.
%
% Rationale:
% A model implementation in the form of code is discrete by nature.
% TargetLink therefore supports only discrete-time blocks, so that
% potential continuous-time parts of a controller model need to be
% discretized before production code generation.
%
% PASS/FALL CRITERIA:
% ================
% A pass is given if no block of continuous library, except the blocks
% given in the parameter dSPACE.checkds0019.AllowedBlocks, is used,
% otherwise fail is given.
%
% Note: The blocks listet in dSPACE.checkds0019.AllowedBlocks must exactly
% be the same as the corresponding block in the library (i.e. including
% linebreaks, spaces)
% 
% CHECK PARAMETERS:
% =================
% dSPACE.checkds0019.BeingCheckedSLLib:
%   DATATYPE: 
%       string
%   DEFAULT VALUE:
%       'Simulink/Continuous'
...
% 
% dSPACE.checkds0019.AllowedBlocks:
%   DATATYPE: 
%       cellstring
%   DEFAULT VALUE:
%       {''}
...
function [result,cr_items,r_stats] = mes_ds_0019(varargin)
