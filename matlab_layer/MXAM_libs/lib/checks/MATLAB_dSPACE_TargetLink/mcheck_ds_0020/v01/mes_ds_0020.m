% MES check: ds_0020 - Restrictions with regard to the use of blocks from
% the discontinuities library.
%
% function [result,cr_items,r_stats] = mes_ds_0020(system, cmd_s)
%
% RETURN PARAMETERS:
% ==================
...
% MES CHECK: 
% ==========
% ds_0020 - Restrictions with regard to the use of blocks from the
% discontinuities library.
%
% This check tests if only allowed block of discontinuities library is used in
% model.
%
% Rationale:
% TargetLink supports only a restricted subset of blocks to ensure the
% generation of efficient code. The blocks Backlash and Dead Zone are not
% built-in TargetLink blocks but implemented as masked subsystems and
% Custom Code blocks. This might lead to increased effort for fixed-point
% scaling.
%
% PASS/FALL CRITERIA:
% ================
% A pass is given if no block of discontinuities library, except the blocks
% given in the parameter dSPACE.checkds0020.AllowedBlocks, is used,
% otherwise fail is given.
%
% Note: The blocks listet in dSPACE.checkds0020.AllowedBlocks must exactly
% be the same as the corresponding block in the library (i.e. including
% linebreaks, spaces)
% 
% CHECK PARAMETERS:
% =================
% dSPACE.checkds0020.BeingCheckedSLLib:
%   DATATYPE: 
%       string
%   DEFAULT VALUE:
...
function [result,cr_items,r_stats] = mes_ds_0020(varargin)
