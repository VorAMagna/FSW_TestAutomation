% MES check: ds_0050 - Restrictions with regard to the use of blocks from
% the sinks library.
%
% function [result,cr_items,r_stats] = mes_ds_0050(system, cmd_s)
%
% RETURN PARAMETERS:
% ==================
...
% MES CHECK: 
% ==========
% ds_0050 - Restrictions with regard to the use of blocks from the
% sinks library.
%
% This check tests if only allowed block of sinks library is used in
% model.
%
% Rationale:
% TargetLink supports only a restricted subset of blocks to ensure the
% generation of efficient code. It should be noted that some of the
% supported blocks have no impact on code generation but work properly
% during a TargetLink model-in-the-loop simulation.
% PASS/FALL CRITERIA:
% ================
% A pass is given if no block of sinks library, except the blocks
% given in the parameter dSPACE.checkds2131.AllowedBlocks, is used,
% otherwise fail is given.
%
% Note: The blocks listet in dSPACE.checkds0050.AllowedBlocks must exactly
% be the same as the corresponding block in the library (i.e. including
% linebreaks, spaces)
% 
% CHECK PARAMETERS:
% =================
% dSPACE.checkds0050.BeingCheckedSLLib:
%   DATATYPE: 
%       string
%   DEFAULT VALUE:
%       'simulink/Sinks'
...
% 
% dSPACE.checkds0050.AllowedBlocks:
%   DATATYPE: 
%       cellstring
%   DEFAULT VALUE:
...
function [result,cr_items,r_stats] = mes_ds_0050(varargin)
