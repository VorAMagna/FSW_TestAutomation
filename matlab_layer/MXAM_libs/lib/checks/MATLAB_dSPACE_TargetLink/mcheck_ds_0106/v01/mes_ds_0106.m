% MES check: ds_0106  - Use of Data Types for Look-Up Table und Look-Up
% Table (2D) Blocks
%
% function [result,cr_items,r_stats] = <mes_yourcheckid>(system, cmd_s)
%
% INPUT PARAMETERS:
% =================
...
% MES CHECK: 
% ==========
% ds_0106  - Use of Data Types for Look-Up Table und Look-Up Table (2D) Blocks
%
% Floating-point data types for Look-Up Table and Look-Up Table (2D)
% blocks must not be used in combination with the "Equidistant with" option.
% This is because TargetLink currently does not support floating-point 
% data types in combination with equidistant breakpoint data.
% Purpose: Code generation with TargetLink.
%
% PASS/FAIL CRITERIA:
% ===================
% A pass is given, if only Look-Up Table and Look-Up Table(2D) blocks are
% used, where the option "Equidistant with" is not used or if it is used, 
% the x-axis', y-axis' and outputs data type is not floating point. 
...
function [result,cr_items,r_stats] = mes_ds_0106(system,cmd_s)  %#ok<INUSD>
