...
%
% Description:
% Checks the linked block`s port.
%
% Pass-Fail Criteria:
% Linked block`s port has not the same lsb than the outer port.
% -------------------------------------------------------------------------

%% IMPLEMENTATION
function [result,cr_items,r_stats]=mes_us_0008(system,cmd_s)			% type before the id the keyword mes_
%% STEP 1: Pre-processing: Getting check info (assumes that check description is available)
[reg,s_regerr]=mxam_getCheckInfoFromDB('us_0008'); 						% get defaults from db
...
try
    ...
    default_item=struct('result','faulty','qualifier','block','linkaction','hilite',...
        'message','Linked block`s port has not the same lsb than the outer port');
                ...
                for x=1:length (TL_Ports)
                    if strcmp((get_param((TL_Ports{x}),'MaskType')),'TL_Inport')	%  search for a TL_Inport in the linked block
                        ...
                        if(size(OutsideInportConnect)==y)
                            if~(tl_get(OutsideInportHandler,'Output.lsb')==tl_get(TL_Ports{x},'Output.lsb'))	% if LSB not same rule violation
                                ...
                                it=mxam_newcritem(default_item,...
                                    'message','Input lsb is not simular to the outer input lsb',...
                                    'handle',bh,...
                    ...
                    if strcmp((get_param((TL_Ports{x}),'MaskType')),'TL_Outport')	% search the connected port to the TL_Outport
                        Pcounter=Pcounter+1;
                        if Pcounter <= length(Ports)
                            OutsideOutport=Ports(Pcounter);
                            OutsideOutportHandler = getfield(OutsideOutport,'DstBlock');
                            ...
                            if(size(OutsideOutportConnect)==y)
                                if~(tl_get(OutsideOutportHandler,'Output.lsb')==tl_get(TL_Ports{x},'Output.lsb'))	% if LSB not same rule violation
                                    bh=get_param(TL_Ports{x},'Handle');
                                    ...
                                    % build check item structure for current rule violation: (predefined start)
                                    it=mxam_newcritem(default_item,...
                                        'message','Output lsb is not simular to the outer output lsb',...
                                        'handle',bh,...
                                        'path',mes_cleanpathname(bh));
                                    cr_items{end+1} = it; % add new check item to result list (predefined end)
                                end;
