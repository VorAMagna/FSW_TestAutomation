...
%
% Description:
% Checks the Terminator background color.
%
% Pass-Fail Criteria:
% Terminator background color has to be light blue.
% -------------------------------------------------------------------------

%% IMPLEMENTATION
function [result,cr_items,r_stats] = mes_us_0003(system,cmd_s)
...
[reg,s_regerr] = mxam_getCheckInfoFromDB('us_0003');  %#ok<NASGU> % <-- ENTER YOUR CHECK-ID HERE !
...
try
   ...
   cs_blocks = find_system(model_name, ...
                        'LookUnderMasks', 'all',...
                        'FollowLinks','on',...
                        'BlockType', 'Terminator');  % <-----   SET YOUR SEARCH CRITERIA HERE !!!
    ...
    % set custom check_param options:
    % (dont change name of r_cp_ops)
