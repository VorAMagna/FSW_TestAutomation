...
model_name = gcs;
mFileName = mfilename;
mFileName = mFileName(1:end-length('_test'));
...
%     end
    ...
    if(B)
        ...
        if BlockIsExcluded == 1
            disp([Name,' is excluded.']);
        end
        ...
        if ~isempty(lookupmeth)
            if (lookupmeth~=2 && ~BlockIsExcluded)		%control of the values (if true rule violation)
                ...
                disp([Name,': Look-Up method is not Interpolation-Use End Values.',blkExcludeFdbk,...
                    char(10),'Block handle: ',b]);
                
%                 it=mxam_newcritem(default_item,...
%                     'message','Look-Up method is not Interpolation-Use End Values',...
%                     'handle',bh,...
