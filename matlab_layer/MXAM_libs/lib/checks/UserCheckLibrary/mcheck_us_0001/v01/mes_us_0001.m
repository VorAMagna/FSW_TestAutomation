...
%
% Description:
% Checks the input block background color.
%
% Pass-Fail Criteria:
% Input block background color has to be yellow. 
% -------------------------------------------------------------------------

%% IMPLEMENTATION
function [result,cr_items,r_stats] = mes_us_0001(system,cmd_s)	% type before the id the keyword mes_
...
[reg,s_regerr] = mxam_getCheckInfoFromDB('us_0001');% <-- ENTER YOUR CHECK-ID HERE !
...
try
   ...
   %% STEP 3: YOUR CUSTOM CODE PART STARTS HERE...
    ...
    cs_blocks = find_system(model_name, ...
                        'FollowLinks','on',...
                        'BlockType', 'Inport');  % <-----   SET YOUR SEARCH CRITERIA HERE !!!
    ...
    % (progressbar will be updated in mes_periprocess and check_param)
    ...
    [fixlog,cr_items] = check_param(cs_blocks,'BackGroundColor','yellow','Background color',r_cp_ops); 
