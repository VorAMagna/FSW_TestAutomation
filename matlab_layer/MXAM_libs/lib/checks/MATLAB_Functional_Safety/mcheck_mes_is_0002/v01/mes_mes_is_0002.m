% MES check: mes_is_0002 - User specified state/transition execution order
%
% function [result,cr_items,r_stats] = mes_mes_is_0002(system, cmd_s)
%
% RETURN PARAMETERS:
% ==================
...
% MES CHECK: 
% ========== 
% mes_is_0002 The execution order of parallel states
% and of transitions must be specified by the user, otherwise
% misinterpretations w.r.t. the semantics of the chart may occur. For that
% purpose it is mandatory to activate the Stateflow chart option 'User
% specified state/transition execution order'.
%
% PASS/FAIL CRITERIA
% ==================
% The check analyzes each Stateflow Chart in the model.
%   - Failed1: The transition order in the chart is not set to be user-specified.
...
function [result,cr_items,r_stats] = mes_mes_is_0002(varargin) %#ok<INUSD>
