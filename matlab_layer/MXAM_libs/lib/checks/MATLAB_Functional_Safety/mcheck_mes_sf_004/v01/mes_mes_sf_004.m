% MES check: mes_mes_sf_004 - Freeness of early return logic problems
%
% function [result, cr_items] = mes_mes_sf_004(system, cmd_s)
%
% RETURN PARAMETERS:
% ==================
...
% Early return logic is an optimisation feature of the SF chart
% interpretation algorithm. For an explanation of Early Return Logic
% problems see: Stateflow, User Guide, Stateflow Chart Semantics, Early
% Return Logic for Event Broadcasts.
%
% PASS CRITERIA:
% ================
% A Fail will be given if in the actions of a state, the command that
% issues an event is not located at the end of the actions, or if a
% transition issues an event that is used as condition by the transitions
% originated from the same source state or its parent states.
% For all other cases a pass will be given.
%
% NOTE
% ====
% The criteria defined above is currently found the author, other cases my
% exist and appended later.
%
% ***********************************************************
...
function [result, cr_items, r_stats] = mes_mes_sf_004(system, cmd_s)
