% MES check: misra_slsf_053_j - Stateflow transition appearance
%
% function [result, cr_items] = mes_misra_slsf_053_j(system, cmd_s)
%
% RETURN PARAMETERS:
% ==================
...
% MES CHECK:
% ==========
% misra_slsf_053_j - Stateflow transition appearance.
% In all statflow flowcharts and graphical functions, there must be only
% one terminating junction.
%
% PASS CRITERIA:
% ================
% This check analyzes each flow-chart and graphical function in the model.
%     Passed 1: There is one and only one terminating junction per
%     flow-chart/graphical function, that is a junction that has input
%     transition(s) and no output transition.
...
function [result, cr_items, r_stats] = mes_misra_slsf_053_j(system, cmd_s)
