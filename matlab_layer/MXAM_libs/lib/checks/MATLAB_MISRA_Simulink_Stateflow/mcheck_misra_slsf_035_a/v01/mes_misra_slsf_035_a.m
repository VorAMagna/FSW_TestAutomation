% MES check: misra_slsf_035_a - Usage of state-charts and flow-charts in a
% state machine
%
% function [result, cr_items] = mes_misra_slsf_035_a(system, cmd_s)
%
% RETURN PARAMETERS:
% ==================
...
% MES CHECK:
% ==========
% misra_slsf_035_a - Usage of state-charts and flow-charts in a
% state machine.
%
% The choice of whether to utilize a state-chart or flow-chart is driven by the
% nature of the behaviour being modelle. A flow-chart must not alter its
% behaviour based on the results of its previous executions. If such
% behaviour is required, a state-chart must be used. 
%
% State behaviour is more easily understood when drawn as a state-chart.
% Functionality is more readable when drawn as a flow chart. 
%
% PASS/FAIL CRITERIA:
% ===================
% A warning is given, when flow-charts are detected. User has to check
% manually, if this chart alter its behaviour based on the results of its
% previous executions.
%
%
% ***********************************************************
...
function [result, cr_items, r_stats] = mes_misra_slsf_035_a(system,cmd_s)
