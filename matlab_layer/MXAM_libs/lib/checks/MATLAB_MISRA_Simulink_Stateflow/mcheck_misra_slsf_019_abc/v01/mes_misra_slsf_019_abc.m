% MES check: misra_slsf_019 - Simulink window appearance
%
% function [result,cr_items,r_stats] = mes_misra_slsf_019(system, cmd_s)
%
% RETURN PARAMETERS:
% ==================
...
%
% A) All Simulink windows should have a white screen color
% B) The zoom factor should be 100%. For models that exceed the Simulink
%    window at 100% zoom, the "fit system to view option should be used
% C) For the menu options in the Simulink window relating to 'view' and 
%    'format' that should be set on completion of the model, 
%    see Appendix D -- Simulink window appearance settings.
%
%
%
% RATIONALE:
% ==========
% To ensure good readability when printed and photocopied, and when viewed
% on development computers in vehicles.
%
% PASS CRITERIA:
% ================
% A pass is given to the model if all appearance settings 
% are conform with the settings given in parameter SystemSetting.
%
% FAIL CRITERIA:
% ================
% A fail is given to the model if any appearance settings deviates from
% the settings in parameter SystemSetting
%
% GUIDELINE REFERENCE:
% ====================
% Misra AC SLSF 019, Version 1.0
%
% DEVIATION FROM GUIDELINE:
% ========================
% The following settings are NOT tested by the check:
% because currently, Simulink provides no API:
%     The need to use the 'fit system to view' option.
%     View Library browser (should be off).
%     View model explorer (should be off).
%     View Port values (should be 'show when hovering').
%
% CHECK PARAMETERS:
% =================
% MISRA_SL_SF.SystemSetting:
%   DATATYPE: 
%       cellstring
%   DEFAULT VALUE:
...
%        'WideLines','on'}
%   DESCRIPTION:
%       Simulink System Settings must be set according to values in this
%       parameter. 
% 
% ***********************************************************
% Copyright:    Model Engineering Solutions
% Date:         $Date: 2020/07/31 16:04:11CEST $
% Revision:     $Rev: 6836 $
% ***********************************************************

function [result,cr_items,r_stats] = mes_misra_slsf_019_abc(varargin) 
