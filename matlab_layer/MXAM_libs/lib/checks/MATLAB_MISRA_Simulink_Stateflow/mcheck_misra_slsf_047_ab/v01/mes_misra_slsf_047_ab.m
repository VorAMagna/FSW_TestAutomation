% MES check: misra_slsf_047 - Stateflow events
%
% function [result, cr_items] = mes_misra_slsf_047(system, cmd_s)
%
% RETURN PARAMETERS:
% ==================
...
% MES CHECK:
% ==========
% misra_slsf_047 - Stateflow events
% A. Local, directed and broadcast Stateflow events, including all implicit
%    events (e.g. change, enter, exit),must not be used. Exception:
%    temporal logic.
% In Stateflow, different types of events can be specified:
%    . A local event occurs anywhere in the Stateflow chart. It is only
%      visible in its parent object and the descendants of the parent object.
%    . A directed event broadcast uses, for example, the send(event_name,
%      state_name) syntax to directly send event event_name to the state
%      state_name. . An implicit event is a built-in event that occurs as a
%      chart executes. These events are not defined or triggered explicitly.
%      The following implicit events exist.
%         . the chart wakes up
%         . entry into a state
%         . exit from a state
%         . a value is assigned to an internal data object
% B. Output Stateflow events must only be used as an output and not tested
%    internally on transition conditions.
%
% PASS CRITERIA:
% ================
% A chart receives a pass if following conditions are fulfiled:
% 1. no local event is define
% 2. no implicit events (entry en exit ex, change chg) is used in chart
% 3. output event is not used as contition
% othewise, the chart receives a fail.
%
% NOTE
% ====
% simulink input event can be used, tick(wakeup) event can be used.
%
% ***********************************************************
...
function [result, cr_items, r_stats] = mes_misra_slsf_047_ab(system, cmd_s)
