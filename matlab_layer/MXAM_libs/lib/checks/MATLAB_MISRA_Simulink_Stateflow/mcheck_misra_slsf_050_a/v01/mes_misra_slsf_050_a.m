% MES check: misra_slsf_050_a - Stateflow window and text appearance
%
% function [result, cr_items] = mes_misra_slsf_050_a(system, cmd_s)
%
% RETURN PARAMETERS:
% ==================
...
% DESCRIPTION:
% ==========
% misra_slsf_050_a - All Stateflow text should use the Arial font.
%
% PASS/FAIL CRITERIA:
% ===================
% The check analyzes every text in a Stateflow chart.
% Failed1: If the font name of transition labels is not 'Arial'.
% Failed2: If the font name of state labels is not 'Arial'.
% The check result is passed if all failed cases do not apply. 
%
% SOLUTION:
% =========
% Use the font 'Arial' for transition labels, state labels and annotations 
% (notes). R2011b: Edit -> Style R2013b: File -> Stateflow Preferences -> Style. 
% Choose the font per right-cklick on the transition resp. the state label. 
% Annotations have the same font as the state label. 
% 
% FIX OPERATION:
% ==============
% The automatic repair action sets the font of every transition label, state label and annotation (note) to 'Arial'. 
% 
% CHECK PARAMETERS:
% =================
% No parameters used.
...
function [result, cr_items, r_stats] = mes_misra_slsf_050_a(system,cmd_s)
