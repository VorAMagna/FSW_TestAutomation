% MES check: misra_slsf_041_a - Usage of boxes in Stateflow.
%
% function [result, cr_items] = mes_misra_slsf_041_a(system, cmd_s)
%
% RETURN PARAMETERS:
% ==================
...
% MES CHECK:
% ==========
% misra_slsf_041_a - Usage of boxes in Stateflow
% Boxes must only contain text.
%
% Additionally, flow charts could be allowed within boxes by setting
% parameter MISRA_SL_SF.AllowFlowChartInBox to true.
%
% When boxes are used as a grouping mechanism, they can be confused with
% states and can also unintentionally modify with the execution order of
% parallel states.
% The functionality of boxes is not clearly documented.
%
% PASS/FAIL CRITERIA:
% ===================
% The check analyzes each box in charts.
% Failed: If anything else than annotations are placed within a box.
%
% CHECK PARAMETERS:
% =================
% MISRA_SL_SF.misra_slsf_041_a.AllowFlowChartInBox:
%   DATATYPE:
%       boolean
%   DEFAULT VALUE:
%       false (0)
%   DESCRIPTION:
%       If FALSE: Boxes must only contain text.
%       If TRUE: Flowcharts may be positioned within boxes (a flowchart
%       consists only of transitions and junctions.).
%
% ***********************************************************
...
function [result, cr_items, r_stats] = mes_misra_slsf_041_a(system,cmd_s)
