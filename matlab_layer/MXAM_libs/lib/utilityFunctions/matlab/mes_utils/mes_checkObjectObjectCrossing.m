% [h_cross,errmsg] = mes_checkObjectObjectCrossing(lh_objects1,lh_objects2)
%
% returns handles of objects in lh_objects1 which cross objects in
...
% if it crosses the other objects in lh_objects1
%
% OUTPUT:
% h_cross: Matrix of handles, h_cross consists of two columns. Each row
%          returns handles of an objects of lh_objects1 (column1) which
%          crosses an object of lh_objects2 (column2, same row).
...
function [h_cross,errmsg] = mes_checkObjectObjectCrossing(lh_objects1,lh_objects2,tol,diag_tol) %#ok<INUSD>
